...
const Layout: React.FC<LayoutProps> = ({ children, user, currentPage, setCurrentPage, onLogout, onLoginClick, onPostRequest, notifications = [], onMarkRead, onMarkAllRead, onToggleAi }) => {
  ...
  const SidebarLink = ({ page, label, icon, onClick, badge }: { page?: string, label: string, icon: React.ReactNode, onClick?: () => void, badge?: number }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-800">
      ...
      <header className="hidden md:flex h-16 bg-white border-b border-gray-200 fixed top-0 left-0 right-0 z-50 items-center px-6 justify-between shadow-sm">
        ...
        <div className="flex items-center w-64">
          ...
          <div className="flex items-center cursor-pointer" onClick={() => setCurrentPage('home')}>
            <div className="w-8 h-8 bg-dubai-gold rounded-lg mr-3 flex items-center justify-center text-white font-bold text-lg">D</div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 h-[80px] pb-safe safe-area-pb grid grid-cols-5 items-center px-1 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
      ...
      {isMobileMenuOpen && user && (
        <>
          ...
          <div className="md:hidden fixed bottom-[85px] left-2 right-2 bg-white rounded-2xl z-[70] animate-in slide-in-from-bottom duration-200 border border-gray-100 shadow-2xl safe-area-pb mb-2">
